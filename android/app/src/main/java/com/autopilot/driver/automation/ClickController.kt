...
class ClickController {
    ...
    suspend fun click(target: TargetDetection, screenWidth: Int, screenHeight: Int): ClickResult {
        val safe = target.safeRegion ?: return ClickResult(false, "Target has no safe region")
        val x = safe.centerX()
        val y = safe.centerY()
        if (!isValidCoordinate(x, y, safe, screenWidth, screenHeight)) {
    ...
    private fun isValidCoordinate(x: Int, y: Int, region: Rect, width: Int, height: Int): Boolean =
        x in 0 until width && y in 0 until height && region.contains(x, y)
}