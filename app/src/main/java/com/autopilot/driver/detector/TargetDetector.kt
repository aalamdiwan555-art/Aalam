...
class TargetDetector(
    ...
    fun detect(
        ...
        val match = text.asSequence()
            .filter { nowMs - it.timestampMs <= MAX_FRAME_AGE_MS }
    ...
    private fun matchScore(value: String, phrase: String): Float {
        ...
        if (normalizedValue.split(' ').size != normalizedPhrase.split(' ').size ||
            normalizedPhrase.length < 5 || normalizedValue.length < normalizedPhrase.length ||
            normalizedValue.length > normalizedPhrase.length + 2
        ) return 0f
        return if (levenshtein(normalizedValue, normalizedPhrase) <= 1) 0.8f else 0f
    }
