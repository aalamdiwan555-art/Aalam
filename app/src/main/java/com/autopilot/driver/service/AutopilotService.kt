...
class AutopilotService : Service() {
    ...
    private fun onFrame(bitmap: android.graphics.Bitmap, geometry: CaptureGeometry) {
        val generation = sessionGeneration
        ...
        processingJob = scope.launch {
            ...
            try {
                frameMutex.withLock {
                    ...
                    if (shouldClick && clickKey != null) {
                        val action = clickController.click(result.target, geometry)
                        currentCoroutineContext().ensureActive()
                        synchronized(lifecycleLock) {
                ...
                synchronized(lifecycleLock) {
                    if (generation == sessionGeneration) actionInFlight = false
                }
    ...
    private fun publishError(message: String, fatal: Boolean = false) {
        snapshot = snapshot.copy(
            ...
            diagnostics = snapshot.diagnostics.copy(
                ...
                ocr = DiagnosticState.ERROR,
                openCv = DiagnosticState.ERROR,
                lastMessage = message,
            ),
        )
    ...
    private fun notification(text: String): Notification =
        NotificationCompat.Builder(this, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_stat_autopilot)
